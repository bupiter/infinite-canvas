export default {
    ...
    voteWorkbench: {
        dataNoticeTitle: "数据与密钥说明",
        settingsDescription: "配置 Vote 生图专用密钥并管理当前浏览器中的工作台数据",
        dataNotice: "生图 API Key 仅保存在当前浏览器本地，并只会作为 Authorization 请求头发送到 https://image.vote520.com。画布、参考图和生成结果也保存在本机浏览器中，不会通过 URL 导入密钥。请勿在公共设备上保存 Key。",
        keyTitle: "生图专用 API Key",
        apiLabel: "固定 API",
        modelLabel: "固定模型",
        verifyConnection: "验证连接",
        connectionVerified: "连接验证通过，已保存生图专用 Key",
        authenticationFailed: "Key 无效或已失效，请重新创建生图专用 Key",
        groupNotImageOnly: "该 Key 不是仅开放 gpt-image-2 的生图专用分组",
        serviceUnavailable: "暂时无法连接生图服务，请稍后重试",
        firstUseTitle: "确认数据处理与计费说明",
        firstUseModeration: "提示词会发送给内容安全审核服务和生图上游。",
        firstUseStorage: "生成图片会在私有对象存储中临时保留，最长 2 天。",
        ...
        clearAll: "清除 Key 和本地数据",
        clearAllTitle: "清除所有本地工作台数据？",
        clearAllDescription: "这会删除当前浏览器保存的 API Key、画布、参考图片、生成图片、任务和历史记录，且无法恢复。",
        about: "关于 Vote 无限画布",
        aboutDescription: "Vote 无限画布基于开源项目 Infinite Canvas 二次开发，并遵循 MIT License。",
        upstream: "查看 Infinite Canvas 上游项目",
    },
    locale: {
        zhCN: "简体中文",
        enUS: "English",
