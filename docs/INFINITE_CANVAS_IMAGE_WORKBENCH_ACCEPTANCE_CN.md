...
2026-08-10 使用最终合并提交 `29290b9d` 对应的运行时代码完成一次最小浏览器闭环。`1fae6df` 到 `29290b9d` 之间只有验收文档变化，运行时代码无差异；本地镜像仅把正式图片 API Origin 临时替换为 `http://127.0.0.1:18082`，该替换层不得进入生产镜像。

- 生图专用 Key 连接验证通过。
- 文生图异步任务约 4 秒完成，生成结果可从生成记录恢复。
- 刷新后生成记录仍存在，选择记录后图片可重新显示。
- 图片加入“我的资产”后写入 IndexedDB，刷新资产页后仍可读取。
- 发现 `/assets` SPA 路由与物理静态目录 `/assets/` 冲突：修复前直接访问或刷新返回 404；增加精确 SPA 路由后，直接访问和再次刷新均通过。
- 数据库仅产生 1 条使用记录、1 个不同请求 ID、1 张计费图片，实际成本 0.134，无重复计费。
- MinIO 最近 15 分钟仅新增 1 个对象；对应 Redis 任务状态为 `completed`，任务 JSON 为 743 字节，不含 Base64，活动任务锁为 0。
- 本地测试环境对该测试用户未产生 `prompt_audit_events`，因此本轮不作为“真实审核恰好一次”的运行证据；该项继续由单元测试覆盖，并必须在生产灰度用真实审核配置复核。
- 一次性测试 Key 已禁用，测试用户余额已从 3.97294385 恢复为 4.10694385。
...
- `/assets` 与 `/assets/` 精确 SPA 路由：Nginx 配置检查通过，直接访问与刷新均返回工作台资产页。
