...
- 已推送 Head：`5d8e12a`
- 本地未推送内容：`03bdbaf` 及其后续，包含 Key 验证竞态修复、移动端布局修复、安全守卫和本验收文档。
...
- 已推送 Head：`453e7353`
- 本地未推送内容：`03fc03b5`、`95d1bfc3`、`e9b2fefa` 及其后续，包含审核故障关闭测试、Redis WATCH 冲突重试和稳定性测试。
...
2、4、8、12、20 总并发均完成三批。2、4、8 档当时通过，但原始 P50/P95 输出未单独保存为 D 盘文件，生产验收前应重新形成可归档结果。
...
| 并发生产值经过压测 | 部分通过 | 12/20 三批通过；生产实际值与 2/4/8 归档指标仍待最终确认。 |
| 回滚不影响主站与文本 | 部分通过 | 图片网关严格 404 文本/管理路由，组件可独立停用；尚未执行生产回滚演练。 |

## 6. 进入生产灰度前必须完成

1. 提交并推送本轮 Canvas 移动端修复与新增守卫，更新 PR #2。
2. 提交并推送 Sub2API 审核故障关闭和 Redis 冲突重试改动，更新 PR #13。
3. 审阅并合并两个 PR，构建不可变生产镜像。
4. 依据生产升级文档备份、部署，并核对 `canvas.vote520.com`、`image.vote520.com`、私有对象存储和生命周期。
5. 使用真实 Pro 完成中文文字、文生图、单图和多图参考测试。
6. 模拟两个指定 Pro 同时故障，确认直接失败且不跨组。
7. 设置短签名 URL 做实际到期测试，确认刷新后仍从 IndexedDB 读取。
8. 重新归档 2/4/8/12/20 每档三批的成功率、429/5xx、P50/P95、账号状态、内存、对象上传和计费结果。
9. 完成不少于 100 张真实生产灰度，无错误扣费、跨组调度、Token 泄露或 Pro 账号异常后，再将第 24 节状态改为完成。
