...
+ [新增] Vote 生图线路支持 Sub2API 异步任务提交、轮询、单 Key 单任务约束和页面重开恢复。
+ [新增] Vote 无限画布增加内嵌模式、专用 API Key 本地管理、一键数据清除和 MIT 上游说明。
+ [新增] Vote 生图支持 1K 源图经 Sub2API Lanczos 重采样为 2K、4K 或合法自定义尺寸，并支持 1 至 10 张顺序生成。
+ [调整] Vote 生图固定专用 API、`gpt-image-2` 和低画质 1K 源图，并禁用 URL 密钥导入、插件远程代码、Agent、视频、音频及无效参数入口。
