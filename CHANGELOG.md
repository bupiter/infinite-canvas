...
- [修复] 修复放大导出弹窗按钮点击可能被画布手势层拦截的问题。
- [修复] 修复放大尺寸自动推荐逻辑会覆盖用户选择的问题，并提高选中态文字对比度。

- [优化] 放大导出支持指定宽高、裁剪填满和透明留白，并在 Worker 中处理大图。

- [优化] Vote 生图增加跨页面排队与可靠任务恢复，按任务编号确认本机保存；原图优先展示，放大导出独立运行，并精简无效参数与按需加载页面。

+ [修复] 修复刷新或配置变化时已完成的异步生图任务可能永久转圈或误报失败，并恢复 R2 图片的跨域尺寸处理与真实错误显示。

- 新用户自动获得 Vote 生图与 Vote 文本模型渠道，旧用户缺少时自动补齐；允许 Canvas 连接 ai.vote520.com，用文本 Key 为反推提示词节点选择语言模型。
- Vote 生图改用异步任务提交与轮询，移除同步请求断线后的整单重试，避免 Cloudflare 超时触发重复生图和重复计费。

+ [优化] Vote 生图专线改为同步直连并注入实测有效的比例构图约束；瞬时网络错误仅重试一次，已返回的有效图片在方向或尺寸增强异常时仍会交付，避免后处理导致整次生成失败。

+ [新增] Vote 无限画布增加内嵌模式、安全的 API Key 本地管理、一键数据清除和 MIT 上游说明。
+ [新增] 内置 4 个 123UQ Image 美甲试戴场景模板；同源加载、默认启用，新老用户均可在提示词库使用。
+ [调整] Canvas 恢复 Infinite Canvas 原生多渠道、模型、画质、尺寸、视频、音频、Agent、插件与图片编辑能力，不再覆盖用户配置。
+ [调整] Canvas 生图恢复标准 OpenAI 兼容同步接口，移除专用异步任务、轮询恢复、固定模型、固定低画质和域名判断分支。
+ [调整] 仅 `image.vote520.com` 的 `gpt-image-2` 专线使用异步任务：上游固定低画质同构图比例源图，Sub2API 以 Lanczos 输出用户选择的准确像素尺寸；其他渠道继续使用各自原生参数和接口。
+ [优化] Vote 生图专线允许连续提交并按同一 API Key 顺序执行，工作台保留全部待处理结果，已受理任务可在刷新后恢复轮询。
+ [安全] 配置导出不包含 API Key，URL 不接受密钥或主站 Token，生产 CSP、私有对象存储和 iframe 来源限制保持不变。
+ [修复] 配置导入统一执行模型与内置提示词迁移，并让 Node 静态服务安全拒绝畸形 URL 和越界静态路径。
